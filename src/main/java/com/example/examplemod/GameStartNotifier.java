...
public class GameStartNotifier {
    ...
    public static void startCountdown(MinecraftServer server, Runnable callback) {
        ...
        public static void onServerTick(TickEvent.ServerTickEvent event) {
            if (event.phase == TickEvent.Phase.END && countdownTicks > 0) {
                ...
                if (countdownTicks % 20 == 0 && countdownTicks <= 5 * 20) {
                    ...
                    sendGameStartMessage(serverInstance, "게임 시작까지 " + secondsLeft + "초", "모두 준비하세요!");
                }
                // 카운트 다운 종료
                if (countdownTicks <= 1) {
                    onCountdownEnd.run();
        ...
        private static void sendGameStartMessage(MinecraftServer server, String title, String subtitle) {
    ...
    private static void sendTitle(ServerPlayer player, String title, String subtitle, int duration) {
